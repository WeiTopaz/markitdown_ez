...
| 值 | 行為 |
|------|------|
| `page_closed`（**預設**） | 啟動後保留 60 秒寬限期；寬限期過後若從未收到 `/heartbeat` 或心跳已停超過 30 秒，視同網頁已關閉並退出。 |
...
2. 前端每 15 秒送出心跳；後端依 `MARKITDOWN_WATCHDOG_MODE` 判定何時自動退出（預設 `page_closed`，60 秒寬限期後若無心跳即退出）
