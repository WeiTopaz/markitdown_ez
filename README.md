...
頁面右下方可按「關閉服務」結束應用程式；關閉瀏覽器頁籤時也會嘗試自動退出，但瀏覽器或作業系統強制終止時不保證送達。

#### 環境變數 `MARKITDOWN_WATCHDOG_MODE`

控制後端如何判定「網頁已關閉」並自動退出，未設定則使用 `page_closed`：

| 值 | 行為 |
|------|------|
| `page_closed`（**預設**） | 啟動後保留 60 秒寬限期；寬限期過後若從未收到 `/heartbeat`，視同網頁從未成功開啟並退出。頁面關閉或按「關閉服務」時由 `/shutdown` 退出，不因靜置心跳逾時退出。 |
| `timeout` | 維持舊版行為：每 10 秒檢查，距上次心跳超過 30 秒即退出。**無啟動寬限期**。 |

設為其他值會印出 warning 並自動 fallback 至 `page_closed`。
...
2. 前端開啟後立即送出一次心跳，之後每 15 秒補送；後端依 `MARKITDOWN_WATCHDOG_MODE` 判定何時自動退出（預設 `page_closed`，60 秒寬限期後若從未收到心跳即退出）
3. 按「關閉服務」會呼叫 `/shutdown`；瀏覽器關閉時也會透過 `beforeunload` 嘗試呼叫 `/shutdown`，應用程式延遲 1 秒後退出
